...
function Chat() {
  ...
  const { idInstance, apiTokenInstance, activeContact } = useContext(UserContext);

  const handleFormSubmit = (event) => {
    event.preventDefault();
    sendMessage(inputValue, idInstance, apiTokenInstance);
    setInputValue('');
  };

  async function sendMessage(message, idInstance, apiTokenInstance) {
    ...
    const response = await fetch(url, {
      ...
      body: JSON.stringify({
        chatId: `${activeContact.number}@c.us`,
        message: message,
      }),
    });
  ...
  useEffect(() => {
    async function getMessage() {
      ...
      if (result) {
        await fetch(
          `https://api.green-api.com/waInstance${idInstance}/deleteNotification/${apiTokenInstance}/${result.receiptId}`,
          { method: 'DELETE' },
        );
  ...
  }, [idInstance, apiTokenInstance, receivedMessages]);
        ...
        <ul>
          {messages.length > 0 &&
            messages.map((message, index) => {
              return (
                <li key={index} className="chat__list">
                  {message.message}
                </li>
              );
            })}
          ...
          {messages.length > 0 &&
            receivedMessages.map((message, index) => {
              return (
                <li key={index} className="chat__list chat__list-left">
                  {message.message}
                </li>
              );
            })}
      ...
      <form className="chat__form" onSubmit={handleFormSubmit}>
        ...
        <button type="submit" className="button">
          <svg
            ...
            <defs>
              <clipPath id="clip0">
                <rect fill="white" height="134" transform="translate(0.777344)" width="157" />
              </clipPath>
            </defs>
          </svg>
        </button>
